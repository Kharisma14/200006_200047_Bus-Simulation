...
void busstop();
void pine();
void bus_text(float, float);
void drawWheel();
void building_scal();
void building3_abd_bottom();
void building3_scal();
void building3_translate();
...
//fungsi menggerakkan kendaraan
void vehicles_moving(){
    //untuk menggerakkan mobil kecil
    if(bool_car){
...
GLvoid Tire(GLdouble y)
{
	...
	glBegin(GL_POLYGON);
    for(i=0;i<=7;i+=.01)
        glVertex2f(y+15+3*cos(i),15+3*sin(i));
	glEnd();
...
void bus_text(float x,float y){
    char string2[]="UNIVERSITAS PADJADJARAN";
...
int main()
{
    ...
    glutCreateWindow("BUS STOP SIMULATION");
    init();
